        ...
        {consultList && consultList.reverse().slice(0, 2).map((consult, index) => {
          return (
            <ScheduleWrapper key={index} style={{ marginTop: '20px' }}>
              ...
              <ScheduleDateAndTime>
                <TextBody2>{new Date(consult.Date).getFullYear()}년 {new Date(consult.Date).getMonth() + 1}월 {new Date(consult.Date).getDate()}일({getDayInKorean(new Date(consult.Date))})</TextBody2>
                <TextSubtitle1>
